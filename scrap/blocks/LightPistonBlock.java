...
public class LightPistonBlock extends DirectionalBlock {
    ...
    public LightPistonBlock(boolean sticky) {
        super(AbstractBlock.Properties.create(Material.PISTON).hardnessAndResistance(1.5F));
    ...
    private void checkForMove(World worldIn, BlockPos pos, BlockState state) {
        ...
        if (flag && !state.get(EXTENDED)) {
            if ((new LightPistonBlockStructureHelper(worldIn, pos, direction, true)).canMove()) {
            ...
            int i = 1;
            if (blockstate.matchesBlock(RegisterPistons.lightMovingPiston) && blockstate.get(FACING) == direction) {
                TileEntity tileentity = worldIn.getTileEntity(blockpos);
                if (tileentity instanceof LightPistonTileEntity) {
                    LightPistonTileEntity pistontileentity = (LightPistonTileEntity)tileentity;
    ...
    public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
        ...
        if (id == 0) {
            ...
            if (tileentity1 instanceof LightPistonTileEntity) {
                ((LightPistonTileEntity)tileentity1).clearPistonTileEntity();
            }

            BlockState blockstate = RegisterPistons.lightMovingPiston.getDefaultState().with(LightMovingPistonBlock.FACING, direction).with(LightMovingPistonBlock.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT);
            worldIn.setBlockState(pos, blockstate, 20);
            worldIn.setTileEntity(pos, LightMovingPistonBlock.createTilePiston(this.getDefaultState().with(FACING, Direction.byIndex(param & 7)), direction, false, true));
            ...
            if (this.isSticky) {
                ...
                boolean flag1 = false;
                if (blockstate1.matchesBlock(RegisterPistons.lightMovingPiston)) {
                    TileEntity tileentity = worldIn.getTileEntity(blockpos);
                    if (tileentity instanceof LightPistonTileEntity) {
                        LightPistonTileEntity pistontileentity = (LightPistonTileEntity)tileentity;
    ...
    private boolean doMove(World worldIn, BlockPos pos, Direction directionIn, boolean extending) {
        ...
        if (!extending && worldIn.getBlockState(blockpos).matchesBlock(RegisterPistons.lightPistonHead)) {
            worldIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 20);
        }

        LightPistonBlockStructureHelper pistonblockstructurehelper = new LightPistonBlockStructureHelper(worldIn, pos, directionIn, extending);
        if (!pistonblockstructurehelper.canMove()) {
            ...
            for(int l = list.size() - 1; l >= 0; --l) {
                ...
                worldIn.setBlockState(blockpos3, RegisterPistons.lightMovingPiston.getDefaultState().with(FACING, directionIn), 68);
                worldIn.setTileEntity(blockpos3, LightMovingPistonBlock.createTilePiston(list1.get(l), directionIn, extending, false));
                ablockstate[j++] = blockstate5;
            }

            if (extending) {
                PistonType pistontype = this.isSticky ? PistonType.STICKY : PistonType.DEFAULT;
                BlockState blockstate4 = RegisterPistons.lightPistonHead.getDefaultState().with(LightPistonHeadBlock.FACING, directionIn).with(LightPistonHeadBlock.TYPE, pistontype);
                BlockState blockstate6 = RegisterPistons.lightMovingPiston.getDefaultState().with(LightMovingPistonBlock.FACING, directionIn).with(LightMovingPistonBlock.TYPE, this.isSticky ? PistonType.STICKY : PistonType.DEFAULT);
                map.remove(blockpos);
                worldIn.setBlockState(blockpos, blockstate6, 68);
                worldIn.setTileEntity(blockpos, LightMovingPistonBlock.createTilePiston(blockstate4, directionIn, true, true));
            }
            ...
            if (extending) {
                worldIn.notifyNeighborsOfStateChange(blockpos, RegisterPistons.lightPistonHead);
            }
